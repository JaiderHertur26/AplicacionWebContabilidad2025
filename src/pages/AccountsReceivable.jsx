...
import { Plus, Download, Edit2, Trash2, Search, CheckCircle, DollarSign, ClipboardList, Printer, Loader2, Lock } from 'lucide-react';
...
const AccountsReceivable = () => {
    ...
    const handleSaveReceivable = (receivableData) => {
        ...
        if (isNew) {
            ...
            const voucherNumber = getNextVoucherNumber('income'); // Provision gets a number too
            ...
            const newIncomeTransaction = {
                ...
                destination: 'pending_receivable', // Explicit Pending Destination
    ...
    const handleMarkAsCollected = (paymentData) => {
        ...
        // Generate NEW voucher number for the collection event
        const newVoucherNumber = getNextVoucherNumber('income');
        ...
        const updatedTransactions = transactions.map(t => {
            if (t.id === `txn-inc-${receivable.id}`) {
                return {
                    ...t,
                    destination: destination, // Change Pending -> Real Cash Dest
                    date: format(new Date(), 'yyyy-MM-dd'), // Update to collection date
